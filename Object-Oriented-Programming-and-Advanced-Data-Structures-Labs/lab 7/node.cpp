...
namespace coen79_lab7
{
    ...
    void list_tail_insert(node*& tail, const std::string &newName, const float &newPrice) {
        assert(tail != NULL && newName.compare("") != 0);   //preconditions
        node* new_tail = new node(newName, newPrice, NULL);
        tail->setLink(new_tail);    //makes new node and sets current tail pointer link to the new node and makes new node the tail
        tail = new_tail;
    }
    
    void list_clear(node*& head) {
        while(head != NULL){        //cycles through list removing each element
