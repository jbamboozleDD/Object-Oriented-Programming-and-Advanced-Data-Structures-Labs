...
/*
 * Description: Sets pivot as element at highest index in array, then loops through array
 * moving elements to the front of the array that are less than the pivot. Once it has gone 
 * through the entire array it puts the pivot behind the highest index element that is smaller
 * than the pivot.
 *
 * Complexity: O(n)
 */
int partition(void **elts, int low, int high, SET *sp)
{
    assert(elts != NULL);
    int j;
    void *temp;
...
/*
 * Description: Picks element and paritions other elements around it in increasinly smaller segments
 * until the whole array is sorted.
 *
 * Complexity: O(nlog(n))
 */
void quicksort(void **elts, int low, int high, SET *sp)
{
    assert(elts != NULL);
    if(low >= high)
        return;
    int pi = partition(elts, low, high, sp);
    quicksort(elts, low, pi -1, sp);
    quicksort(elts, pi + 1, high, sp);
}
